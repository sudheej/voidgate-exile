...
export default function Tile(scene) {
    ...
    this.createTile = function (type) {
        let rectangle = new Phaser.GameObjects.Rectangle(this.scene, TileProperties.x, TileProperties.y, TileProperties.width, TileProperties.height, 0x880000);
        let entryGradient = new Phaser.GameObjects.Graphics(this.scene);
        if (type === "path") {
            rectangle.setFillStyle(0x000000)
        }
        else if (type === "start_path") {
